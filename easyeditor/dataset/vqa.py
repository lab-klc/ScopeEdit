...
class VQADataset(BaseDataset):
    def __init__(self, data_dir: str, size:  typing.Optional[int] = None, config=None, *args, **kwargs):
        ...
        self.is_llava = (
            config is not None
            and hasattr(config, "model_name")
            and "llava" in config.model_name.lower()
            and "onevision" not in config.model_name.lower()
        )
        vis_processor = None if self.is_llava else BlipImageEvalProcessor(
            image_size=364, mean=None, std=None
        )
    ...
    def collate_fn(self, batch):
        ...
        raw_trg = [b['target'] for b in batch]
        trg = [" " + t for t in raw_trg]
        ...
        raw_loc_a = [b["locality_ground_truth"] for b in batch]
        loc_a = [" " + t for t in raw_loc_a]
        m_loc_image = [b['multimodal_locality_image'] for b in batch]
        m_loc_q = [b['multimodal_locality_prompt'] for b in batch]
        raw_m_loc_a = [b['multimodal_locality_ground_truth'] for b in batch]
        m_loc_a = [" " + t for t in raw_m_loc_a]
        
        # edit_inner
        edit_inner = {}
        edit_inner['image'] = image if self.is_llava else torch.stack(image, dim=0)
        edit_inner['text_input'] = [self.prompt.format(s) + t for s, t in zip(src, trg)]
        edit_inner['labels'] = trg
        edit_inner['prompt_text'] = [self.prompt.format(s) for s in src]
        edit_inner['target_text'] = raw_trg
        ...
        # edit_outer
        edit_outer = {}
        edit_outer['image'] = image if self.is_llava else torch.stack(image, dim=0)
        edit_outer['text_input'] = [self.prompt.format(r) + t for r, t in zip(rephrase, trg)]
        edit_outer['labels'] = trg
        edit_outer['prompt_text'] = [self.prompt.format(r) for r in rephrase]
        edit_outer['target_text'] = raw_trg
        if self.config.model_name == "minigpt4" or self.config.model_name == "blip2":
        ...
        # edit_outer_image
        edit_outer_image = {}
        edit_outer_image['image'] = (
            image_rephrase if self.is_llava else torch.stack(image_rephrase, dim=0)
        )
        edit_outer_image['text_input'] = [self.prompt.format(s) + t for s, t in zip(src, trg)]
        edit_outer_image['labels'] = trg
        edit_outer_image['prompt_text'] = [self.prompt.format(s) for s in src]
        edit_outer_image['target_text'] = raw_trg
        if self.config.model_name == "minigpt4" or self.config.model_name == "blip2":
        ...
        loc['labels'] = loc_a
        loc['prompt_text'] = loc_q
        loc['target_text'] = raw_loc_a
        if self.config.model_name == "minigpt4" or self.config.model_name == "blip2":
        ...
        loc_image = {}
        loc_image['image'] = (
            m_loc_image if self.is_llava else torch.stack(m_loc_image, dim=0)
        )
        loc_image['text_input'] = [self.prompt.format(q) + a for q, a in zip(m_loc_q, m_loc_a)]
        loc_image['labels'] = m_loc_a
        loc_image['prompt_text'] = [self.prompt.format(q) for q in m_loc_q]
        loc_image['target_text'] = raw_m_loc_a
        if self.config.model_name == "minigpt4" or self.config.model_name == "blip2":
