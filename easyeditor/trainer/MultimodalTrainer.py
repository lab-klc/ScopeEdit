...
class MultimodalTrainer(BaseTrainer):
    ...
    def edit_step(self, batch, training: bool):
        ...
        def _masked_token_consistency(pre_logits, post_logits, mask):
            if mask is None:
                return None
            denom = mask.float().sum()
            if denom.item() <= 0:
                return None
            pre_ids = pre_logits.argmax(dim=-1)
            post_ids = post_logits.argmax(dim=-1)
            return ((pre_ids == post_ids) & mask).float().sum() / denom
        ...
        def _should_accumulate_locality_keys():
            if not hasattr(self.model, "accumulate_current_keys"):
                return False
            if training and not bool(
                getattr(self.config, "more_accumulate_locality_keys_in_training", False)
            ):
                return False
            return True
        ...
        with torch.no_grad():
            ...
            loc_answer_mask = _answer_mask_from_outputs(base_outputs, base_logits)
            loc_image_answer_mask = _answer_mask_from_outputs(base_image_outputs, base_image_logits)
        ...
        if not isinstance(model_info, dict):
            model_info = {}
        active_groups = model_info.get("more/top_groups")
        key_info = {}
        self.last_edited_model = edited_model
        ...
        with torch.set_grad_enabled(training):
            ...
            if _should_accumulate_locality_keys():
                _merge_numeric_info(
                    key_info,
                    self.model.accumulate_current_keys(active_groups, source="loc"),
                )
            ...
            if _should_accumulate_locality_keys():
                _merge_numeric_info(
                    key_info,
                    self.model.accumulate_current_keys(active_groups, source="loc_image"),
                )
            ...
            l_loc_answer = (
                kl_loc_loss(base_logits.detach(), post_base_logits, mask=loc_answer_mask)
                if loc_answer_mask is not None
                else None
            )
            l_image_loc_answer = (
                kl_loc_loss(base_image_logits.detach(), post_image_base_logits, mask=loc_image_answer_mask)
                if loc_image_answer_mask is not None
                else None
            )
        ...
        loc_acc_answer = _masked_token_consistency(base_logits, post_base_logits, loc_answer_mask)
        if loc_acc_answer is not None:
            info_dict["loc/acc_answer"] = loc_acc_answer.item()
        image_loc_acc_answer = _masked_token_consistency(base_image_logits, post_image_base_logits, loc_image_answer_mask)
        if image_loc_acc_answer is not None:
            info_dict["image_loc/acc_answer"] = image_loc_acc_answer.item()
        if l_loc_answer is not None:
            info_dict["loss/loc_answer"] = l_loc_answer.item()
        if l_image_loc_answer is not None:
            info_dict["loss/image_loc_answer"] = l_image_loc_answer.item()
        ...
        restore_fn = None
        if "restore_fn" in model_info:
            restore_fn = model_info.pop("restore_fn")
        info_dict = {**info_dict, **model_info, **key_info}
    ...
    def _inline_validation_log(self, step, stats, start_time, steps):
        ...
        loc_acc_answer = f"{stats.get('loc/acc_answer_val', float('nan')):<12.5f}"
        loc_image_acc_answer = f"{stats.get('image_loc/acc_answer_val', float('nan')):<12.5f}"

        LOG.info(
          f"Step {prog} outer_acc: {outer_acc} image_acc: {image_acc} inner_acc: {inner_acc} it_time: {elapsed:.4f} loc_acc: {loc_acc}, image_loc: {loc_image_acc}, loc_acc_ans: {loc_acc_answer}, image_loc_ans: {loc_image_acc_answer}"
        )
