    ...
    def average(self):
        ...
        for k, v in self.underlying.items():
            ...
            if not k.startswith("nll/"):
                scalars = []
                for item in v:
                    val = _maybe_scalar(item)
                    if val is None:
                        average[k] = v[-1]
                        break
                    scalars.append(val)
                else:
                    average[k] = sum(scalars) / len(scalars)
            else:
                ...
                nll_vals = []
                count_vals = []
                for nll, c in zip(v, token_counts):
                    nll_val = _maybe_scalar(nll)
                    c_val = _maybe_scalar(c)
                    if nll_val is None or c_val is None:
                        average[k] = v[-1]
                        break
                    nll_vals.append(nll_val)
                    count_vals.append(c_val)
                else:
                    total_nll = sum([nll * c for nll, c in zip(nll_vals, count_vals)])
                    average[k] = total_nll / sum(count_vals)
                    if self.compute_ppl:
