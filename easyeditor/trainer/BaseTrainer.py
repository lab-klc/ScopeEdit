...
class BaseTrainer:
    def __init__(self, config, train_set: Dataset, val_set: Dataset):
        ...
        if ('minigpt4' in self.config.model_name.lower()
                or 'blip2' in self.config.model_name.lower()
        ...
        if hasattr(self.model, "initialize_basis"):
            self.model.initialize_basis(self.train_loader)
    ...
    def save_state(self, stats):
        if not self.config.save or self.config.eval_only:
            return
    ...
    def echo(self, train_step, info_dict, pretty=False):
        if not self.config.silent:
            ...
            LOG.info(
                sep.join([f"{key_format(k)}: {value_format(v)}" for k, v in info_dict.items()])
            )
